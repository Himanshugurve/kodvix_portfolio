...
function MobileCard({ num, initial, name, tag, year, stack, desc, accent, metric }) {
  ...
  return (
    <div
      ...
      style={{
        position: "relative", borderRadius: "14px", padding: "1.1rem",
    ...
    >
      ...
      <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"0.65rem" }}>
        <div style={{ width:"36px", height:"36px", borderRadius:"9px", flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", border: hov?`1px solid ${accent}65`:"1px solid rgba(255,255,255,0.1)", background: hov?`${accent}14`:"rgba(255,255,255,0.04)", boxSizing:"border-box", transition:"all 0.3s" }}>
          <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:"1rem", lineHeight:1, color:hov?accent:"rgba(255,255,255,0.65)", transition:"color 0.3s" }}>{initial}</span>
        </div>
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <h3 style={{ fontFamily:"'Barlow',sans-serif", fontWeight:700, fontSize:"0.95rem", lineHeight:1.15, letterSpacing:"-0.01em", color:hov?"#f0fdff":"#e2e8f0", margin:0, transition:"color 0.3s" }}>{name}</h3>
        </div>
        <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:500, fontSize:"0.52rem", letterSpacing:"0.16em", textTransform:"uppercase", color:accent, border:`1px solid ${accent}35`, background:`${accent}0d`, padding:"3px 8px", borderRadius:"999px", flexShrink:0, display:"inline-flex", alignItems:"center" }}>{tag}</span>
      </div>
      <div style={{ height:"1px", background:`linear-gradient(90deg,${accent}40,transparent)`, marginBottom:"0.65rem", opacity:hov?1:0.3, transition:"opacity 0.3s" }} />
      <p style={{ fontFamily:"'Lora',serif", fontWeight:300, fontSize:"0.82rem", lineHeight:1.68, color:hov?"rgba(203,213,225,0.85)":"rgba(100,116,139,0.8)", margin:"0 0 0.65rem 0", transition:"color 0.3s" }}>{desc}</p>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"8px" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"5px", flexWrap:"wrap" }}>
          {stack.map((s,i) => <StackChip key={i} label={s} accent={accent} />)}
        </div>
        <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:"2px" }}>
          <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:"1.3rem", lineHeight:1, color:accent, textShadow:`0 0 14px ${accent}55` }}>{metric.val}</span>
...
function DesktopRow({ num, initial, name, tag, year, stack, desc, accent, metric, isTablet }) {
  ...
  return (
    <div
      onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ position:"relative", display:"grid", gridTemplateColumns:cols, borderBottom:"1px solid rgba(255,255,255,0.055)", overflow:"hidden", alignItems:"stretch", cursor:"default" }}
    >
      ...
      <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center", padding:isTablet?"1.25rem 0 1.25rem 1rem":"1.6rem 0 1.6rem 1.5rem", gap:"8px" }}>
        ...
        <div style={{ width:isTablet?"32px":"36px",height:isTablet?"32px":"36px",borderRadius:"9px",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,border:hov?`1px solid ${accent}65`:"1px solid rgba(255,255,255,0.1)",background:hov?`${accent}14`:"rgba(255,255,255,0.04)",transition:"all 0.3s",boxSizing:"border-box" }}>
          <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:"1rem",lineHeight:1,color:hov?accent:"rgba(255,255,255,0.65)",transition:"color 0.3s",display:"block" }}>{initial}</span>
        </div>
      </div>
      <div style={{ padding:isTablet?"1.25rem 1rem":"1.6rem 1.5rem",display:"flex",flexDirection:"column",justifyContent:"center",gap:"7px",minWidth:0 }}>
        <div style={{ display:"flex",alignItems:"center",gap:"8px",flexWrap:"wrap" }}>
          <h3 style={{ fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:isTablet?"0.9rem":"clamp(0.9rem,1.5vw,1.1rem)",lineHeight:1.1,letterSpacing:"-0.01em",color:hov?"#f0fdff":"#e2e8f0",margin:0,transition:"color 0.3s" }}>{name}</h3>
          <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:500,fontSize:"0.55rem",letterSpacing:"0.18em",textTransform:"uppercase",color:accent,border:`1px solid ${accent}35`,background:`${accent}0d`,padding:"3px 9px",borderRadius:"999px",lineHeight:"1.4",display:"inline-flex",alignItems:"center",flexShrink:0 }}>{tag}</span>
        ...
        <p style={{ fontFamily:"'Lora',serif",fontWeight:300,fontSize:isTablet?"0.78rem":"clamp(0.76rem,1.1vw,0.84rem)",lineHeight:1.7,color:hov?"rgba(203,213,225,0.85)":"rgba(100,116,139,0.8)",margin:0,transition:"color 0.3s",maxWidth:"520px" }}>{desc}</p>
        <div style={{ display:"flex",alignItems:"center",gap:"5px",flexWrap:"wrap" }}>
          {stack.map((s,i) => <StackChip key={i} label={s} accent={accent} />)}
        </div>
      </div>
      <div style={{ display:"flex",flexDirection:"column",alignItems:"flex-end",justifyContent:"center",padding:isTablet?"1.25rem 1rem 1.25rem 0.5rem":"1.6rem 1.6rem 1.6rem 0.75rem",gap:"10px",flexShrink:0,borderLeft:"1px solid rgba(255,255,255,0.04)" }}>
        <div style={{ display:"flex",flexDirection:"column",alignItems:"flex-end",gap:"3px",opacity:hov?1:0.45,transition:"opacity 0.3s" }}>
          <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:isTablet?"1.1rem":"clamp(1.25rem,2.2vw,1.7rem)",lineHeight:1,color:accent,textShadow:`0 0 18px ${accent}55` }}>{metric.val}</span>
  ...
  return (
    <div className="pf-root">
      <style>{`
        ...
        /*
         * ROOT FIX:
         * - NO min-height: 100vh  → section height = content height only
         * - NO overflow: hidden   → nothing is ever clipped
         * - Generous padding-top  → heading is always fully visible
         */
        .pf-root {
          position: relative;
          width: 100%;
          ...
          /* overflow must stay visible so nothing clips */
          overflow: visible;
          padding: 6rem 4rem 5rem;
        }

        /* header sits naturally in flow — no absolute positioning */
        .pf-header {
          position: relative;
          z-index: 10;
          margin-bottom: 2rem;
        ...
        .pf-rows      { position: relative; z-index: 10; display: flex; flex-direction: column; }
        ...
        @media (max-width: 1023px) {
          .pf-root        { padding: 5rem 2rem 4rem; }
          .pf-header      { margin-bottom: 1.75rem; }
          .pf-col-header  { grid-template-columns: 58px 1fr 120px; }
          .pf-stats       { gap: 1.25rem; padding: 10px 16px; }
          .pf-footer      { margin-top: 1.25rem; }
        }

        /* ── MOBILE (≤599px) ── */
        @media (max-width: 599px) {
          .pf-root { padding: 4rem 1rem 3rem; }
          .pf-header { margin-bottom: 1.25rem; }
          ...
          .pf-footer       { justify-content: center; margin-top: 1rem; }
          .pf-footer-tags  { display: none; }
        }
      `}</style>

      {/* BG blobs — clipped to the element itself via the bg colour */}
      <div style={{ position:"absolute",top:0,left:0,right:0,bottom:0,pointerEvents:"none",zIndex:0,overflow:"hidden",borderRadius:0 }}>
        <div style={{ position:"absolute",top:"-12%",right:"-6%",width:"520px",height:"520px",borderRadius:"50%",background:"radial-gradient(circle,rgba(8,145,178,0.07) 0%,transparent 70%)",filter:"blur(80px)" }} />
        <div style={{ position:"absolute",bottom:"-10%",left:"-6%",width:"440px",height:"440px",borderRadius:"50%",background:"radial-gradient(circle,rgba(30,58,138,0.07) 0%,transparent 70%)",filter:"blur(80px)" }} />
        <div style={{ position:"absolute",inset:0,backgroundImage:"linear-gradient(rgba(34,211,238,0.022) 1px,transparent 1px),linear-gradient(90deg,rgba(34,211,238,0.022) 1px,transparent 1px)",backgroundSize:"55px 55px" }} />
      </div>
      ...
      <div className="pf-header">
        {/* Section label */}
        <div className="pf-label" style={{ display:"flex",alignItems:"center",gap:"10px",marginBottom:"0.85rem" }}>
        ...
        <div className="pf-heading-row">
          <div>
            <h2 className="pf-h2" style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,textTransform:"uppercase",fontSize:"clamp(1.8rem,5vw,3.8rem)",lineHeight:1.02,letterSpacing:"-0.01em",color:"#ffffff",margin:0,marginBottom:"0.5rem" }}>
              Completed{" "}
              <span style={{ color:"#22d3ee",textShadow:"0 0 24px rgba(6,182,212,0.45),0 0 60px rgba(6,182,212,0.2)" }}>Deployments</span>
            </h2>
            <p className="pf-sub" style={{ fontFamily:"'Lora',serif",fontWeight:300,fontStyle:"italic",fontSize:"clamp(0.82rem,1.4vw,1rem)",letterSpacing:"0.015em",color:"rgba(148,163,184,0.72)",margin:0,lineHeight:1.55 }}>
