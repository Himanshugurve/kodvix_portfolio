  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hov?"0 8px 40px rgba(6,182,212,0.12),0 2px 8px rgba(0,0,0,0.4),inset 0 1px 0 rgba(255,255,255,0.03)":"0 2px 12px rgba(0,0,0,0.3),inset 0 1px 0 rgba(255,255,255,0.02)",
    ...
    >
      ...
      <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:"12px",marginBottom:"0.9rem" }}>
        <div style={{ flex:1,minWidth:0 }}>
          <span style={{ fontFamily:"'Martian Mono',monospace",fontSize:"0.5rem",letterSpacing:"0.24em",color:accent,opacity:0.55,display:"block",marginBottom:"5px",fontWeight:400 }}>{id}</span>
          <h3 style={{ fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:"clamp(0.9rem,1.4vw,1.05rem)",lineHeight:1.2,letterSpacing:"-0.01em",color:hov?"#f0fdff":"#dde6f0",margin:0,marginBottom:"4px",transition:"color 0.3s" }}>{title}</h3>
          <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:400,fontSize:"0.58rem",letterSpacing:"0.18em",color:"rgba(100,116,139,0.58)",textTransform:"uppercase" }}>{tagline}</span>
        </div>
        {/* Icon — strict 42×42 */}
        <div style={{ width:"42px",height:"42px",flexShrink:0,borderRadius:"10px",display:"flex",alignItems:"center",justifyContent:"center",backgroundColor:"rgba(8,47,73,0.5)",border:hov?`1px solid ${accent}99`:"1px solid rgba(6,182,212,0.2)",color:accent,filter:hov?`drop-shadow(0 0 8px ${accent}80)`:"none",transition:"all 0.35s",boxSizing:"border-box" }}>
          {icon}
        </div>
      </div>

      {/* Divider */}
      <div style={{ width:"100%",height:"1px",background:`linear-gradient(90deg,${accent}40,transparent)`,marginBottom:"0.9rem",opacity:hov?1:0.4,transition:"opacity 0.3s" }} />
      ...
      <div style={{ display:"flex",flexDirection:"column",gap:"0.8rem",flex:1 }}>
        {features.map((f,i) => (
          <div key={i} style={{ display:"flex",alignItems:"flex-start",gap:"10px" }}>
            ...
            <div style={{ flex:1,minWidth:0 }}>
              <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"0.64rem",fontWeight:600,letterSpacing:"0.16em",color:"rgba(203,213,225,0.9)",textTransform:"uppercase",display:"block",marginBottom:"3px" }}>{f.label}</span>
              <span style={{ fontFamily:"'Lora',serif",fontWeight:300,fontSize:"clamp(0.75rem,1.1vw,0.84rem)",lineHeight:1.68,color:"rgba(100,116,139,0.82)",display:"block",letterSpacing:"0.01em" }}>{f.desc}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Status */}
      {extra === "status" && (
        <div style={{ marginTop:"1.1rem",padding:"9px 14px",borderRadius:"8px",display:"flex",alignItems:"center",justifyContent:"space-between",backgroundColor:"rgba(8,47,73,0.3)",border:"1px solid rgba(6,182,212,0.12)" }}>
          <span style={{ fontFamily:"'Martian Mono',monospace",fontSize:"0.46rem",letterSpacing:"0.18em",color:"rgba(34,211,238,0.55)",textTransform:"uppercase",fontWeight:400 }}>System_Status</span>
    ...
    <div className="svc-root">
      <style>{`
        ...
        .svc-root {
          ...
          /* KEY: top padding ensures header is never clipped when used inside a page */
          padding: 4.5rem 4rem 3rem;
        }
        @media (max-width:1023px) { .svc-root { padding: 3.5rem 2rem 2.5rem; } }
        @media (max-width:599px)  { .svc-root { padding: 2.5rem 1rem 2rem; } }

        /* ── Header ── */
        .svc-header { position:relative;z-index:10; margin-bottom:2.75rem; }
        @media (max-width:1023px) { .svc-header { margin-bottom:2rem; } }
        @media (max-width:599px)  { .svc-header { margin-bottom:1.5rem; } }

        /* ── Heading row ── */
        .svc-hrow { display:flex;align-items:flex-end;justify-content:space-between;flex-wrap:wrap;gap:1rem; }
        @media (max-width:599px) { .svc-hrow { flex-direction:column;align-items:flex-start;gap:1rem; } }

        /* ── Badge ── */
        .svc-badge { display:flex;align-items:center;gap:10px;padding:8px 16px;border:1px solid rgba(34,211,238,0.15);border-radius:999px;background:rgba(8,47,73,0.15);backdrop-filter:blur(8px);flex-shrink:0; }
        @media (max-width:599px) { .svc-badge { align-self:flex-start; } }
        ...
        /* ── Footer ── */
        .svc-footer { position:relative;z-index:10;margin-top:2.5rem;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:0.75rem; }
        @media (max-width:1023px) { .svc-footer { margin-top:1.75rem; } }
        @media (max-width:599px)  { .svc-footer { margin-top:1.25rem;justify-content:center; } }

        .svc-tags { display:flex;align-items:center;gap:8px;flex-wrap:wrap; }
        @media (max-width:599px) { .svc-tags { display:none; } }
      ...
      <div className="svc-header">
        ...
        <div className="svc-hrow">
          <div>
            <h2 className="svc-h2" style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:"clamp(1.8rem,5vw,3.8rem)",lineHeight:1.02,letterSpacing:"-0.01em",color:"#ffffff",margin:0,marginBottom:"0.6rem",textTransform:"uppercase" }}>
              Integrated Service{" "}
              <span style={{ color:"#22d3ee",textShadow:"0 0 24px rgba(6,182,212,0.45),0 0 60px rgba(6,182,212,0.2)" }}>Ecosystem</span>
            </h2>
            <p className="svc-sub" style={{ fontFamily:"'Lora',serif",fontWeight:300,fontStyle:"italic",fontSize:"clamp(0.85rem,1.35vw,1.02rem)",letterSpacing:"0.015em",color:"rgba(148,163,184,0.72)",margin:0,lineHeight:1.55 }}>
          ...
          <div className="svc-badge svc-bdg">
            ...
            <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontSize:"0.7rem",fontWeight:500,letterSpacing:"0.18em",color:"#67e8f9",textTransform:"uppercase" }}>
