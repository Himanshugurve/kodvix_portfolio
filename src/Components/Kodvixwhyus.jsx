...
function FeatureCard({ num, icon, title, desc, accent }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ position:"relative",borderRadius:"14px",padding:"clamp(1.25rem,2vw,1.75rem)",display:"flex",flexDirection:"column",overflow:"hidden",background:hovered?"rgba(10,18,36,0.85)":"rgba(8,14,28,0.55)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",border:hovered?`1px solid ${accent}55`:"1px solid rgba(255,255,255,0.06)",boxShadow:hovered?`0 8px 36px ${accent}12,0 2px 8px rgba(0,0,0,0.4),inset 0 1px 0 rgba(255,255,255,0.03)`:"0 2px 10px rgba(0,0,0,0.3),inset 0 1px 0 rgba(255,255,255,0.02)",transform:hovered?"translateY(-4px)":"translateY(0)",transition:"all 0.38s cubic-bezier(0.34,1.2,0.64,1)",cursor:"default",height:"100%",boxSizing:"border-box" }}>
      ...
      <h3 style={{ fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:"clamp(0.95rem,1.4vw,1.1rem)",lineHeight:1.2,letterSpacing:"-0.01em",color:hovered?"#f0fdff":"#e2e8f0",margin:0,marginBottom:"0.6rem",transition:"color 0.3s" }}>{title}</h3>
      <div style={{ width:"100%",height:"1px",background:`linear-gradient(90deg,${accent}40 0%,transparent 80%)`,marginBottom:"0.75rem",opacity:hovered?1:0.35,transition:"opacity 0.3s" }} />
      <p style={{ fontFamily:"'Lora',serif",fontWeight:300,fontSize:"clamp(0.8rem,1.2vw,0.9rem)",lineHeight:1.72,color:hovered?"rgba(203,213,225,0.9)":"rgba(100,116,139,0.85)",margin:0,transition:"color 0.3s" }}>{desc}</p>
    </div>
  );
}
...
function WideCard() {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ position:"relative",borderRadius:"14px",padding:"clamp(1.25rem,2vw,1.75rem)",display:"flex",alignItems:"center",overflow:"hidden",background:hovered?"rgba(10,18,36,0.85)":"rgba(8,14,28,0.55)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",border:hovered?"1px solid rgba(34,211,238,0.45)":"1px solid rgba(255,255,255,0.06)",boxShadow:hovered?"0 8px 36px rgba(6,182,212,0.12),0 2px 8px rgba(0,0,0,0.4)":"0 2px 10px rgba(0,0,0,0.3)",transform:hovered?"translateY(-4px)":"translateY(0)",transition:"all 0.38s cubic-bezier(0.34,1.2,0.64,1)",cursor:"default",flexWrap:"wrap",gap:"clamp(1rem,2.5vw,2rem)",boxSizing:"border-box" }}>
      ...
      <div style={{ flex:1,minWidth:"180px",paddingRight:"clamp(1rem,2.5vw,2rem)",borderRight:"1px solid rgba(51,65,85,0.4)" }}>
        <h3 style={{ fontFamily:"'Barlow',sans-serif",fontWeight:700,fontSize:"clamp(1rem,1.6vw,1.2rem)",lineHeight:1.2,letterSpacing:"-0.01em",color:hovered?"#f0fdff":"#e2e8f0",margin:0,marginBottom:"0.45rem",transition:"color 0.3s" }}>On-Time Delivery</h3>
        <p style={{ fontFamily:"'Lora',serif",fontWeight:300,fontSize:"clamp(0.8rem,1.2vw,0.9rem)",lineHeight:1.7,color:hovered?"rgba(203,213,225,0.9)":"rgba(100,116,139,0.85)",margin:0,transition:"color 0.3s" }}>
          Rigorous project management protocols ensure we meet deadlines without compromising quality or performance.
        </p>
      </div>

      <div style={{ display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"4px",flexShrink:0,minWidth:"80px",opacity:hovered?1:0.75,transition:"opacity 0.3s" }}>
        <span style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:"clamp(1.8rem,3vw,2.5rem)",lineHeight:1,color:"#22d3ee",textShadow:"0 0 20px rgba(34,211,238,0.5)" }}>100%</span>
        <div style={{ width:"100%",height:"1px",background:"linear-gradient(90deg,transparent,rgba(34,211,238,0.4),transparent)",margin:"4px 0" }} />
        <span style={{ fontFamily:"'Martian Mono',monospace",fontSize:"0.48rem",letterSpacing:"0.2em",color:"rgba(100,116,139,0.55)",textTransform:"uppercase",fontWeight:300 }}>Commitment</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ position:"relative",width:"100%",minHeight:"100vh",backgroundColor:"#050810",display:"flex",flexDirection:"column",padding:"clamp(2rem,5vw,4rem) clamp(1.5rem,5vw,4rem)",boxSizing:"border-box",overflow:"hidden",color:"#ffffff" }}>
      <style>{`
        ...
        .why-grid{display:grid;grid-template-columns:repeat(2,1fr);grid-template-rows:auto auto;gap:clamp(0.75rem,1.5vw,1.1rem);}
        .why-wide-span{grid-column:1/-1;}
        @media(min-width:1024px){.why-grid{grid-template-columns:repeat(4,1fr);}.why-wide-span{grid-column:1/-1;}}
        @media(max-width:599px){.why-grid{grid-template-columns:1fr;}}
      `}</style>
      ...
      <div style={{ position:"relative",zIndex:10,marginBottom:"clamp(1.75rem,3.5vw,2.75rem)" }}>
        ...
        <div style={{ display:"flex",alignItems:"flex-end",justifyContent:"space-between",flexWrap:"wrap",gap:"1rem" }}>
          <div>
            <h2 className="why-h2" style={{ fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,textTransform:"uppercase",fontSize:"clamp(2rem,5.5vw,4rem)",lineHeight:0.98,letterSpacing:"-0.01em",color:"#ffffff",margin:0,marginBottom:"0.5rem" }}>
              Why Choose{" "}
              <span style={{ color:"#22d3ee",textShadow:"0 0 24px rgba(6,182,212,0.45),0 0 60px rgba(6,182,212,0.2)" }}>KodVix</span>
            </h2>
            <p className="why-sub" style={{ fontFamily:"'Lora',serif",fontStyle:"italic",fontWeight:300,fontSize:"clamp(0.875rem,1.4vw,1.1rem)",letterSpacing:"0.015em",color:"rgba(148,163,184,0.75)",margin:0 }}>
              Why industry leaders partner with us.
            </p>
          </div>
          <div className="why-sub" style={{ display:"flex",alignItems:"center",gap:"6px",opacity:0.5 }}>
            {[0,1,2].map(i=><div key={i} style={{ width:"5px",height:"5px",borderRadius:"50%",backgroundColor:"#22d3ee",animation:i===0?"dotPulse 2.5s ease-in-out infinite":"none" }} />)}
      ...
      {/* Grid */}
      <div className="why-grid" style={{ position:"relative",zIndex:10,flex:1 }}>
        {features.map((f,i)=>(
          <div key={i} className={`why-card-${i}`} style={{ display:"flex" }}>
            <FeatureCard {...f} />
          </div>
        ))}
        <div className="why-wide-span why-wide"><WideCard /></div>
      </div>

      {/* Footer */}
      <div className="why-footer" style={{ position:"relative",zIndex:10,marginTop:"clamp(1.5rem,3vw,2.25rem)",paddingTop:"clamp(1rem,2vw,1.25rem)",borderTop:"1px solid rgba(30,41,59,0.5)",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:"1rem" }}>
